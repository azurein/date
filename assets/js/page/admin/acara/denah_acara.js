...
$(document).ready(function(){
	...
	$("#btnPrintCanvas").click(function(){
		...
		var judul = $("#txtCanvasName").val();

		$('#tableInfo').css('background','#FFFFFF');
		$('#tableInfo').css('color','#000000');
		$('#tableInfo').css('display','block');
		$('.table-responsive').css('background','#FFFFFF');
		$('.table-responsive').css('padding-left','5%');
		$('.table-responsive').css('padding-top','5%');
		$('#tableInfo').prepend($('#canvasOne'));
		$('#tableInfo th').css('text-align', 'center');
		$('#CanvasBorder').remove();
		$('.input-group-denah').remove();
		$('.panel-group').remove();

		doc.addHTML($('.table-responsive')[0], function () {
			doc.save('Cetak Denah '+judul+'.pdf');
		});

<?php
class Update_model_5 extends CI_Model {
	public function __construct()
	{
		parent::__construct();
		$this->db = $this->load->database('default',TRUE);
		$this->db_2 = $this->load->database('model_5',TRUE);
	}
	...
	public function lastUpdate(){
		...
		$this->db->query($query);
		$this->db_2->get('delegate_verification');
		foreach ($query->result() as $row) {
		...
		$this->db->query($query);
		$this->db_2->get('participant_facility');
		foreach ($query->result() as $row) {
